...
var cloneNode = function(node) {
  ...
  var attribs = clone.attribs = {};
  var data = clone.data = {};
  var attr;

  for (attr in node.attribs) {
    attribs[attr] = node.attribs[attr];
  }
  for (attr in node.data) {
    data[attr] = node.data[attr];
  }
...
// necessary
var makeDomArray = function(elem, i, len) {
  var clone = i < len - 1;
  var nodes;
  ...
  if (elem == null) {
    ...
    nodes = elem.toArray();
    return clone ? nodes.map(cloneNode) : nodes;
  } else if (_.isArray(elem)) {
    nodes = _.flatten(elem.map(makeDomArray));
    return clone ? nodes.map(cloneNode) : nodes;
  } else if (_.isString(elem)) {
    return evaluate(elem);
  } else {
    return [clone ? cloneNode(elem) : elem];
  }
};
  ...
  return function() {
    ...
    if (_.isFunction(elems[0])) {
      return this.each(function(i, el) {
        var dom = makeDomArray(elems[0].call(el, i, this.html()), i, length);
        updateDOM(concatenator(dom, el.children || (el.children = [])), el);
      });
    } else {
      return domEach(this, function(i, el) {
        var dom = makeDomArray(elems, i, length);
        updateDOM(concatenator(dom, el.children || (el.children = [])), el);
      });
    }
  };
...
var after = exports.after = function() {
  ...
  domEach(this, function(i, el) {
    ...
    if (_.isFunction(elems[0])) {
      ...
      dom = makeDomArray(elems, i, length);
    }
...
var replaceWith = exports.replaceWith = function(content) {
  ...
  domEach(this, function(i, el) {
    ...
    if (_.isFunction(content)) {
      ...
      dom = makeDomArray(content, i, length);
    }
